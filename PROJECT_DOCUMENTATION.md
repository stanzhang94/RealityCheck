...
| 最近一次 `dotnet build` | 本次文档整理未运行；之前文档任务曾验证过构建，但不能作为本次游戏内验收 | 本次任务记录 [F] |
| release zip | `Pathoschild.Stardew.ModBuildConfig` 构建时通常会生成；本次未重新生成 | 项目构建依赖和本次任务记录 [P] |
| 游戏内 UI 验证 | 本次未完成游戏内验证 | 本次任务记录 [F] |

当前文档任务没有修改业务代码，也没有改 `manifest.json`、`ModEntry.cs`、`Services/`、`UI/`、`Models/`、`Data/`、`Events/`、`Patches/`。[F]
...
| `1.4.1` | UI layout update，当前 manifest 版本。 | `manifest.json`、git commit `88dac9c` [F] |
| 当前 Unreleased | 删除复杂工作流文档，改为 `main` 单线和唯一中文总文档。 | 本次任务 [F] |
...
- 当前 `1.4.1` 在游戏内的完整 UI 验收尚未在本次任务中执行。[U]
