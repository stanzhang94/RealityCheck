...
| 最近一次 `dotnet build` | `1.4.3` Financial Manual 拖动滚动与 Exchange 快捷金额按钮已正常构建并部署，0 warning、0 error | 本地构建结果 [F] |
| release zip | 使用正式发布结构生成 `RealityCheck-1.4.2.zip` | 本次发布任务 [F] |
| 游戏内 UI 验证 | `1.4.2` 三项修复已完成人工测试；`1.4.3` 拖动滚动尚需玩家实机验证 | Stan 人工验收与当前任务范围 [F][U] |

`1.4.3` 为 Financial Manual 增加按住鼠标左键或触摸后上下拖动正文滚动，并为 Exchange 转账金额增加触屏快捷按钮；这不是完整 Android UI 适配，不改变财务、税务、市场价格、Exchange 资金口径或存档逻辑。[F]
...
| Financial Manual | 游戏内财务手册，包含日报、季度报、年报、税务和 Market Price 页面；支持鼠标滚轮及正文拖动滚动。 | `UI/FinanceMenu.cs` [F] |
...
`1.4.2` 至 `1.4.3` 相关源码与关键方法：[F]

| 文件 | 关键方法 | 当前职责 |
| --- | --- | --- |
| `Services/MarketPriceService.cs` | `GetShippingBinMarketSellPrice`、`GetShopSaleMarketUnitPrice` | 取得普通品质标准 Market Price，并为玩家持有的具体物品生成最终品质售价。 |
| `Services/MarketPriceService.cs` | `GetNormalQualityVanillaUnitPrice`、`ApplyItemQualityToStandardMarketPrice`、`GetQualityPriceNumerator` | 探测普通品质 fallback，集中应用一次品质倍率，并按正整数价格截断小数。 |
| `Patches/ShopSaleMarketPricePatch.cs` | `ObjectSellToStorePricePostfix` | 将商店出售接入最终 Market Price，同时跳过内部普通品质探测，防止递归。 |
| `Patches/TooltipMarketPricePatch.cs` | `ObjectSellToStorePricePostfix`、`GetMarketUnitPriceSafely` | 将背包和箱子 Tooltip 接入与实际出售一致的最终品质售价。 |
| `Events/IncomeEvents.cs` | `OnDayEnding` | 按最终品质市场价汇总 Shipping Bin，供实际结算、账本和税务使用。 |
| `Services/ExchangeService.cs` | `ExecuteLongDelivery`、`CreateDeliveryItemStacks` | 计算完整多头交割数量，并按物品实际最大堆叠量创建合法 Stack。 |
| `Services/ExchangeService.cs` | `CanInventoryAcceptAllItems`、`TryAddItemsToInventoryAtomically`、`RestoreInventorySnapshot` | 预演背包容量，执行一次性交割，并在意外失败时恢复交割前背包状态。 |
| `Services/ExchangeContractCatalogService.cs` | `SupportsSevenDayRawCrop`、`IsFruitTreeProduct`、`TryGetCropGrowthDaysForHarvestItem` | 从 Stardew Valley 1.6 数据判断地面作物和果树产物的 7 日合约资格。 |
| `UI/FinanceMenu.cs` | `receiveScrollWheelAction`、`leftClickHeld`、`releaseLeftClick`、`ChangeScrollOffset` | 让滚轮和按住拖动共用现有 `scrollOffset`、内容高度与上下边界，并在拖动后阻止 Market Price 行误触。 |
| `UI/ExchangeMenu.cs` | `TryHandleTransferQuickAmountClick`、`AddTransferQuickAmount`、`GetTransferQuickAmountButtonBounds` | 让五个触屏快捷按钮安全修改原有转账金额，并让绘制与点击共用同一组布局矩形。 |
...
`1.4.1` 的问题是：当同一商品已经存在上一日市场历史时，递归市场价格会按统一的 `marketCommodityKey` 返回标准 Market Price。商店、Shipping Bin 和 Tooltip 随后直接使用这个价格，导致普通、银星、金星、铱星具体物品表现为相同售价。[F]

`1.4.2` 将“标准商品市场价格”和“玩家具体物品最终售价”明确分层：[F]

```text
具体物品
→ 创建同一物品的普通品质副本（Quality = 0）
→ 取得普通品质原版 sellToStorePrice fallback
→ 读取或计算普通品质标准 Market Price
→ 只在最终具体物品售价层应用一次当前物品自身的 Quality
```

最终正价格使用整数除法截断小数，不四舍五入：[F]

| Quality | 品质 | 最终售价 |
| ---: | --- | --- |
| `0` | 普通 | `标准 Market Price × 4 / 4`，即标准价不变 |
| `1` | 银星 | `floor(标准 Market Price × 5 / 4)` |
| `2` | 金星 | `floor(标准 Market Price × 6 / 4)` |
| `4` | 铱星 | `floor(标准 Market Price × 8 / 4)` |

无上一日市场历史或商品首次出现时，`GetNormalQualityVanillaUnitPrice` 会先用 `Quality = 0` 的副本取得普通品质 fallback，再计算标准 Market Price，最后才应用当前成品品质。因此不会把已经含品质的 `vanillaUnitPrice` 再乘一次品质倍率。[F]

普通品质标准价格仍是市场历史、Market Price 表、走势图和 Exchange 合约目录的唯一价格。品质不会创建独立 market key，不会写入独立历史，也不会根据玩家背包物品品质改变交易所报价。[F]

品质只读取当前出售或显示成品自身的 `Quality`，不会追溯原材料品质。Fence、Tackle 和不受市场系统管理的特殊物品继续走原版特殊价格路径，不无条件套用品质公式。[F]
...
- 在可滚动正文区域按住鼠标左键或触摸并上下拖动；向上拖查看下方内容，向下拖返回上方内容。
- 拖动与鼠标滚轮共用现有 `scrollOffset`、`GetMaxScrollOffset()` 和绘制偏移，不存在独立 Android 滚动状态。
- 约 8 像素的纵向阈值用于区分点击与拖动；超过阈值后，松手不会触发 Market Price 商品行。
- 关闭按钮、顶部标签、搜索框、排序栏、收藏按钮、Market Price 返回按钮和 Exchange 入口不启动正文拖动。
- 此功能覆盖当前由 Financial Manual 统一滚动偏移控制的 Daily、Seasonal、Annual、Tax、Market Price 列表和商品历史详情。
- 这是基础触屏操作补充，不包含菜单尺寸调整、惯性、回弹、Android 平台判断或完整 Android UI 重构。
...
多头到期后仍使用一次性交割设计，不新增待领取仓库。`CreateDeliveryItemStacks` 先根据合约 `TotalQuantity` 创建交割物品模板，读取该物品实际 `maximumStackSize()`，再把完整数量拆成多个合法 Stack；源码不再假设所有物品都固定为 999，也不再把完整数量写入一个超限 Stack。[F]

正式发货前，`CanInventoryAcceptAllItems` 会在背包副本上模拟：[F]

- 与已有同类物品堆合并后的剩余容量；
- 每个空槽可容纳的合法 Stack；
- 全部交割物品是否能一次性进入背包。

预检查失败时，合约不发生任何财务或物品变化：不发物品、不扣 Exchange 交割款、不写 Long Delivery 成功历史，状态保持 `PendingDelivery`，玩家可以整理背包后再次执行。[F]

预检查通过后，`TryAddItemsToInventoryAtomically` 会保存背包原物品引用和原 Stack，再逐堆加入。如果正式加入期间仍发生异常或返回失败，`RestoreInventorySnapshot` 会恢复交割前背包状态；只有全部物品加入成功后才扣完整交割款、记录完整数量与金额，并最后把合约标记为 `Delivered`。[F]

当前明确不采用的方案：[F]

- 不把多头应收物品写入 `DeliveryStorage`；
- 不支持分批领取；
- 不掉落到地面；
- 不通过邮件补发。

以最大堆叠量 999 的标准商品为例，200 手等于 20,000 件，会拆成 20 堆 999 加 1 堆 20，共 21 堆。999 手等于 99,900 件，需要 100 个完整 Stack，超过原版背包容量时会在预检查阶段完整失败。[F]

### 11.2 1.4.2 的 7 日合约资格

非原始 Fruit/Vegetable 商品继续支持 7 日合约。原始地面作物通过 `Game1.cropData` / `Data/Crops` 匹配 `HarvestItemId`，把各生长阶段 `DaysInPhase` 相加，得到基础首次成熟周期：[F]

```text
首次成熟天数 = sum(DaysInPhase)
首次成熟天数 <= 7 → 支持 7 日合约
首次成熟天数 > 7  → 不支持 7 日合约
```

该判断不计算生长激素、农业学家职业或特殊灌溉缩短时间。可重复收获作物只看第一次成熟周期，不使用 `RegrowDays`。因此未碾米按普通 8 天判断，芋头按普通 10 天判断；芋头仍受现有商品排除表控制，不因此重新进入交易商品池。[F]

果树产物通过 `Game1.fruitTreeData` / `Data/FruitTrees` 的 `FruitTreeData.Fruit` 数据识别，匹配 `ItemId` 和 `RandomItemId`，不使用中文名、英文名或本地化显示名称，也不维护手写水果名单。成熟果树水果全部支持 7 日合约，因为合约对应成熟果树的日常产出，而不是树苗的 28 天成熟期。[F]

代表性资格结果：[F]

| 商品 | 基础生产依据 | 7 日合约 |
| --- | --- | --- |
| Parsnip | 地面作物，首次成熟 4 天 | 支持 |
| Hot Pepper | 地面作物，首次成熟 5 天 | 支持 |
| Cranberries | 地面作物，首次成熟 7 天 | 支持 |
| Apple | 成熟果树产物 | 支持 |
| Strawberry | 地面作物，首次成熟 8 天 | 不支持 |
| Unmilled Rice | 普通地面周期 8 天 | 不支持 |
| Blueberry | 地面作物，首次成熟 13 天 | 不支持 |
| Pumpkin | 地面作物，首次成熟 13 天 | 不支持 |

如果找不到对应 Crop 或 FruitTree 数据，或数据读取发生异常，目录服务会写 Trace 日志并对原始 Fruit/Vegetable 保守返回 `false`，不会让 Exchange UI 崩溃。现有 `ExcludedRawCommodityItemIds`、14 日合约和 28 日合约规则完全不变。[F]
...
7. 在各个可滚动页面分别测试鼠标滚轮与按住拖动，确认顶部/底部边界正常；拖动 Market Price 列表后松手不应打开商品。
8. 打开 Exchange UI，检查账户、创建合约和持仓页面。
9. 过夜，查看 SMAPI 日志、账本变化、税务每日评估和市场价格更新。
10. 在税务结算日检查 Tax Notice 和实际扣款/债务。
...
- 同一种商品的普通、银星、金星、铱星价格差异；
- 已有上一日市场历史，以及无历史/首次出现时不会重复乘品质；
- 背包和箱子 Tooltip、商店出售、Shipping Bin 结算；
- 实际到账、财务账本与税务金额一致；
- 普通品质标准 Market Price 不变，Exchange 标准报价不随品质变化。

大额多头交割测试已覆盖：[F]

- 9 手、10 手、11 手和 200 手的跨 Stack 交割；
- 空背包、容量刚好足够、少一个必要空槽和明显容量不足；
- 失败时没有部分物品、没有扣款、合约保持 `PendingDelivery`；
- 整理背包后可再次交割并取得完整数量；
- 保存退出并读档后，失败的 `PendingDelivery` 合约仍存在。

7 日合约资格测试已覆盖快速成熟地面作物、超过 7 天的地面作物，以及 Apple 等成熟果树产物；结果符合 `Data/Crops` 和 `Data/FruitTrees` 规则，14 日与 28 日资格未发生变化。[F]

上述三组 `1.4.2` 游戏内人工测试均已通过。[F]
...
| `1.4.2` | 修复品质售价、大额多头原子交割，以及快速生长作物和果树产物的 7 日合约资格。 | `manifest.json`、git commits `90e6e61`、`bc8cf2d`、当前源码与人工验收 [F] |
| `1.4.3` | Financial Manual 增加按住鼠标左键或触摸上下拖动正文滚动；Exchange 账户页增加共用原输入金额的快捷按钮。 | `manifest.json`、`UI/FinanceMenu.cs`、`UI/ExchangeMenu.cs` [F] |

### 14.1 Reality Check 1.4.2 正式记录

`1.4.2` 没有增加新的 Exchange 大功能，也没有改变存档数据结构。版本集中处理三个已经确认并完成人工验收的问题：[F]

1. 品质售价：修复商品已有市场历史后不同品质售价相同的问题。普通品质标准 Market Price 保持不变，银星、金星、铱星只在具体物品最终售价层应用一次品质倍率；品质不进入市场历史、走势图和 Exchange 标准报价。Tooltip、商店、Shipping Bin、实际到账、财务账本与税务金额保持一致。
2. 大额多头交割：修复交割数量超过单堆上限时只收到一堆、其余数量丢失的问题。交割按实际 `maximumStackSize()` 拆分并保持一次性原子操作；背包不能完整容纳时不发物品、不扣款、不记录成功，合约保持 `PendingDelivery`。不使用 `DeliveryStorage`，不支持分批领取。
3. 7 日合约资格：恢复快速成熟地面作物和成熟果树产物的 7 日合约。地面作物按首次成熟周期判断，基础周期不超过 7 天才支持；果树水果允许 7 日合约；14 日和 28 日规则不变。

三个修复均已完成游戏内人工测试并通过。对应提交为 `90e6e61 fix: preserve quality prices and prevent bulk delivery loss` 与 `bc8cf2d fix: restore seven-day contract eligibility`。[F]

### 14.2 Reality Check 1.4.3 正式记录

`1.4.3` 是一次范围受限的 Financial Manual 基础触屏适配：[F]

1. Daily、Seasonal、Annual、Tax、Market Price 列表和商品历史详情继续使用原有统一滚动偏移，并新增按住鼠标左键或触摸后上下拖动。
2. 鼠标滚轮行为保留，滚轮与拖动共用同一边界计算；向上拖增加滚动偏移，向下拖减少滚动偏移。
3. 纵向移动达到约 8 像素后才视为拖动。Market Price 商品行点击在未拖动时于松手处理；发生拖动后会吞掉该次松手，避免误选商品。
4. 关闭、标签、搜索、排序、收藏、返回和 Exchange 等固定操作继续走原有点击路径，不启动正文拖动。
5. 本次不调整 UI 布局，不增加惯性、回弹、平台判断或配置项，也不修改财务、税务、市场价格、Exchange、Harmony patch 和存档逻辑。
6. Exchange 账户页继续保留原有自定义金额输入与 PC 实体键盘操作，并在同一个金额输入值上增加 `+100`、`+1K`、`+10K`、`+100K`、`清零` 五个快捷按钮，为 Android 系统键盘未弹出时提供基础金额输入方式。
7. 快捷按钮只修改原有 `transferAmountText`：空值按 0 累加，连续点击可重复累加，清零恢复为空；累加使用 `int` 金额范围并防止溢出，非法文本仍显示原有无效金额提示。
8. 五个快捷按钮使用两行紧凑布局，放在原存入和取出按钮右侧的既有空白中：第一行 3 个、第二行 2 个；绘制与点击共用同一组矩形，原输入框、存入、取出和状态提示位置均不变。
9. 快捷按钮不会直接执行存入或取出。最终转账仍由 `RunTransferFromInput` 调用 `ExchangeService.TryDeposit` 或 `TryWithdraw`，继续分别校验玩家钱包余额和交易所可用现金。

Android 端的触摸事件映射、拖动手感、快捷金额按钮点击和不同设备布局无法仅靠桌面构建确认，仍需玩家在 Android 实机中验证。[U]
...
- `1.4.3` Financial Manual 拖动滚动尚未完成 Android 玩家实机验证；需要确认触摸拖动、边界和松手防误触在实际设备上的表现。[U]
- `1.4.3` Exchange 快捷金额按钮尚未完成 Android 玩家实机验证；需要确认五个按钮的触摸命中、连续累加、清零、布局和原金额输入共存表现。[U]
