...
public class MarketTrendService
{
    ...
    private const string RecursiveDailyPricingModelVersion = "16.7-recursive-daily-v1";
    private const string ShortTrendDurationPricingModelVersion = "16.8-short-trend-duration-v1";
    private const string CurrentPricingModelVersion = "16.9-market-balance-v1";

    private const double SoftLowFinalFactor = 0.75;
    private const double SoftHighFinalFactor = 1.35;
    private const double HardLowFinalFactor = 0.55;
    private const double HardHighFinalFactor = 1.60;
    ...
    )
    {
        ...
        return mode switch
        {
            // RealityCheck 1.3.2 balance fix:
            // Shorter trend durations make the 28-day history chart less one-directional.
            MarketTrendMode.StrongUp => random.Next(1, 4),
            MarketTrendMode.Up => random.Next(2, 6),
            MarketTrendMode.Flat => random.Next(1, 4),
            MarketTrendMode.Down => random.Next(2, 5),
            MarketTrendMode.StrongDown => random.Next(1, 4),
            _ => random.Next(1, 4)
        };
    }
    ...
    private void MigratePricingModelIfNeeded()
    {
        ...
        if (string.Equals(this.data.PricingModelVersion, RecursiveDailyPricingModelVersion, StringComparison.OrdinalIgnoreCase))
        {
            this.ClampExistingTrendDurationsForShortTrendModel();
            this.ResetCurrentTrendDurationsForBalanceModel();
            this.data.PricingModelVersion = CurrentPricingModelVersion;

            this.monitor.Log(
                $"Market trend state migrated to {CurrentPricingModelVersion}; existing price history was preserved, trend durations were shortened, and current trend states will be reselected after the next day update.",
                LogLevel.Info
            );
        ...
        if (string.Equals(this.data.PricingModelVersion, ShortTrendDurationPricingModelVersion, StringComparison.OrdinalIgnoreCase))
        {
            this.ResetCurrentTrendDurationsForBalanceModel();
            this.data.PricingModelVersion = CurrentPricingModelVersion;

            this.monitor.Log(
                $"Market trend state migrated to {CurrentPricingModelVersion}; existing price history was preserved and current trend states will be reselected after the next day update.",
                LogLevel.Info
            );
        ...
        // It cannot be used as yesterday's price for the recursive formula.
    ...
    private void ClampExistingTrendDurationsForShortTrendModel()
    {
        foreach (MarketTrendItemState state in this.data.TrendStates.Values)
        {
            int maxDuration = GetMaxDurationForMode(state.TrendMode);
            state.DaysRemaining = Math.Clamp(state.DaysRemaining, 0, maxDuration);
        }
    }


    private void ResetCurrentTrendDurationsForBalanceModel()
    {
        foreach (MarketTrendItemState state in this.data.TrendStates.Values)
        {
            state.DaysRemaining = 0;
            state.TodayTrendChange = 1.0;
        }
    }

    private static int GetMaxDurationForMode(MarketTrendMode mode)
    {
        return mode switch
        {
            MarketTrendMode.StrongUp => 3,
            MarketTrendMode.Up => 5,
            MarketTrendMode.Flat => 3,
            MarketTrendMode.Down => 4,
            MarketTrendMode.StrongDown => 3,
            _ => 3
        };
    }
